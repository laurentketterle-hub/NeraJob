"""Scraper for wr_programming - public API with offline fallback."""
...
_OFFLINE_KEY = "NERAJOB_WR_PROGRAMMING_OFFLINE"
_OFFLINE_JOBS = [
        ('Rails Developer', 'RubyShop', 'Remote', ['ruby', 'rails', 'postgresql'], 'https://weworkremotely.com/jobs/rails'),
        ('Frontend Developer', 'WebUI', 'Remote', ['javascript', 'vue'], 'https://weworkremotely.com/jobs/frontend'),
    ]


class WrProgrammingScraper(BaseScraper):
    """Public wr_programming job board adapter."""

    name = "wr_programming"
    API_URL = "https://weworkremotely.com/categories/remote-programming-jobs.rss"
    ...
    def _offline_search(self, query, location, limit):
        ...
        for title, company, job_loc, tags, url in _OFFLINE_JOBS:
            ...
            jobs.append(JobPosting(
                id=f"wr_programming-offline-{digest}",
                source=self.name,
                title=title,
                company=company,
                location=job_loc,
